...
pnpm dev:worker           # second terminal; runs on the fake LLM, which .env.example ships
```

`BETTER_AUTH_SECRET` and `INBOUND_EMAIL_SECRET` are the two variables `.env.example` cannot supply, because a value published in this repository is not a secret. The first signs every session cookie; the second is the only credential on `POST /api/webhooks/email/inbound`, so a published one lets anyone manufacture conversations and raise tickets against any client. The web app refuses to start on a blank value, on one under 32 (auth) or 24 (inbound) characters, and on any placeholder shipped here — in every environment, not only production. Set both in your local `.env` before `pnpm dev`; the e2e suite reads `INBOUND_EMAIL_SECRET` from that same file, and the one spec that signs the inbound webhook (`support-intake.spec.ts`) fails by name if it is unset — the rest of the suite is unaffected. On Windows, `node -e "console.log(require('crypto').randomBytes(48).toString('base64'))"` does the same job as `openssl rand -base64 48`.

`pnpm dev:worker` works out of the box because `.env.example` ships `LLM=fake`: a scripted stub (`apps/worker/src/llm/fake.ts`) that needs no `ANTHROPIC_API_KEY`, costs nothing and answers every agent run the same way twice. To switch to real runs, set `ANTHROPIC_API_KEY` in `.env` and change `LLM` to `anthropic` — the worker refuses `LLM=anthropic` with no key in every environment, which is why the shipped default is the fake one. Production is the other way round: `LLM=anthropic` with a key, and `LLM=fake` is refused under `NODE_ENV=production` unless `ALLOW_FAKE_LLM=1` says it was meant.
