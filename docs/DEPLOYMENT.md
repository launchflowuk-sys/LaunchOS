...
4. **Worker resource (Docker build)**
   ...
   | `IMAGEGEN_ADAPTER`, `OPENAI_API_KEY`, `FAL_KEY`, `IMAGEGEN_MONTHLY_CAP_PENCE` | leave **unset** | post images: unset means branded graphics only, and that is a perfectly good place to leave it. See *Post images* |
...
### Post images (`IMAGEGEN_ADAPTER`)

Every scheduled post can carry a picture, and there are two ways of drawing one. The **branded template** — the client's own colours, their wordmark or logo, the headline lifted off the post — is free, needs no key, and is what a post gets by default. The **image generator** draws a photograph instead, and costs money per picture, so it is opt-in per client (Content tab → *AI photography*) and capped.

| Variable | Value | Notes |
|---|---|---|
| `IMAGEGEN_ADAPTER` | `mock`, `openai` or `fal` | selection is **by name**, not by credential — the opposite of the ads adapters next door. This is the one adapter that spends money per call, so an `OPENAI_API_KEY` that arrives for some other reason must not quietly start drawing pictures at fourpence a go; saying `openai` here is how someone says they meant it. A name whose key is missing, and a misspelt name, are both refused in production |
| `OPENAI_API_KEY` | from OpenAI | `gpt-image-1`, quality `medium`: roughly 4p a square image, 6p a rectangle |
| `FAL_KEY` | from fal.ai | Flux Schnell: under a penny an image, recorded as 1p because the cap counts whole pence |
| `IMAGEGEN_MONTHLY_CAP_PENCE` | `2000` (the default) | what the generator may spend across the whole organisation in a Europe/London month. Past it a post gets the branded template instead, and the editor says why — it is a spending limit, not a switch that leaves posts pictureless |

**Leave all four unset and you get branded graphics only, which is a perfectly good place to leave it.** That is not a degraded mode: the template is deliberate, on-brand and identical every time, and most posts look better for it than for a stock-looking photograph. The startup warning that names the mock generator is telling you AI photography is off, not that anything is broken — set `IMAGEGEN_ADAPTER` the day a client asks for it and not before. Both resources want the same values, and both want the same `STORAGE_DIR` volume: a generated image is a content asset like any upload, written under `STORAGE_DIR/content/<org>/` and served from `/api/assets/<uuid>`. There is no third-party image host and nothing to configure at one.
