...
Each integration in `packages/integrations` exports an interface, a `Mock*` implementation and a real client, and `createIntegrations(env)` picks the real one only when its env vars are present: `payments` (Stripe, `PAYMENTS_ADAPTER=stripe` + both secrets), `uptime` (HTTP, `UPTIME_PROBE=http`), `hosting` (Coolify, `COOLIFY_API_URL` + `COOLIFY_API_TOKEN`), `dns` (Hostinger and Cloudflare, `HOSTINGER_API_TOKEN` / `CLOUDFLARE_API_TOKEN`, selected **per domain** from `domains.dns_provider` by a registry that implements `DnsProvider`), `cms` (WordPress, `SECRETS_ENCRYPTION_KEY`, with each site's application password encrypted per site) and `ads` (Google and Meta, **by credential** — all five `GOOGLE_ADS_*` keys, both `META_ADS_*` keys, both together building a router). Agents and core only ever see the interface. The CMS is the one adapter that needs more than env: its credential resolver is bound to an organisation, so `createIntegrations` takes an optional `deps.resolveSiteCredentials`, and the agent registry carries a per-organisation factory (`scopedCmsProvider` in `packages/agents`) that `cms_update_content` calls with the run's own tenant.

**Production refuses a mock it was not meant to run on.** Selection is spread across the factories — `createEmailAdapter` (`packages/channels`), `createPaymentsAdapter`, `createAdsAdapterFromEnv`, `createHostingProviderFromEnv`, `createDnsProvidersFromEnv`, `createCmsProviderFromEnv` and the `UPTIME_PROBE` branch in `createIntegrations` — so `packages/integrations/src/adapter-guard.ts` is the one place that *names* the outcome of all of them. `apps/worker/src/env.ts` and `apps/web/src/lib/env.ts` both call `productionAdapterIssues` before anything connects, and under `NODE_ENV=production` a mock is a refusal unless `ALLOW_MOCK_ADAPTERS=1` says it was meant.

The reasoning is the one already written out for `LLM=fake`, one step worse: a mock adapter does not fail, it *succeeds*. `MockEmailAdapter` returns a provider message id, so `sendQueuedMessage` marks the reply `sent`, stamps `deliveredAt` and audits `message.sent` — a worker deployed without `EMAIL_ADAPTER=smtp` black-holes every client reply, ad report and invoice email while reporting all of them delivered. Three shapes are refused: a mock where the rule is `refuse` (`EMAIL_ADAPTER`, `PAYMENTS_ADAPTER`, `UPTIME_PROBE` — unset counts), a *silent downgrade* where the environment asked for something the factory cannot build and got less (`PAYMENTS_ADAPTER=stripe` with a secret missing, `COOLIFY_API_TOKEN` without its URL, three of the five `GOOGLE_ADS_*` keys, `ADS_ADAPTER=google` with no credentials), and an `UNBUILDABLE` config the factory throws on (`EMAIL_ADAPTER=smtp` without `SMTP_HOST`, a malformed `COOLIFY_API_URL`) — that last one is refused even with the opt-out. The four newer adapters — hosting, DNS, CMS, ads — are **not** refused for being unset (`mockWhenUnset: "log"` on their resolution): production was already running with none of their keys when their real clients landed, and refusing would have refused the next deploy and taught everyone to set the opt-out. Both processes print one `WARN` per tolerated mock (`productionMockWarnings`) and log the resolved names (names only) at startup, so what is actually wired is visible in the first lines of the log.
