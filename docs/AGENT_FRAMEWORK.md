...
If a resume dies partway, the run is left `running`. Nothing in the request path closes it: from there, "an earlier delivery died" and "another delivery is working right now" are the same row. `agent-runs.stuck-sweep` closes it out of band, on the only evidence that separates the two — whether the run is still recording steps. See "When a resume fails".
...
3. **Sweeper.** `approvals.resume-sweep` (cron, every minute, `apps/worker/src/jobs/resume-sweep.ts`) re-enqueues, per organisation, every approval that is `approved`/`rejected`, has a `run_id`, was decided more than 30 seconds ago, and whose run is still `awaiting_approval` **and still parked on that approval's own tool call** — the query joins `agent_runs.metadata -> 'pending' ->> 'awaitingToolUseId'` to `approvals.payload ->> 'toolUseId'`, the same binding `loadParked` enforces. It re-sends under the same `resume:<approvalId>` key. That join is what makes it idempotent by construction: a run that has been claimed (`running`), finished (no `metadata.pending`) or **re-parked on a later tool call** no longer matches. The re-park case is not exotic — a Sentinel run with two flagged accounts, a triage with a reply plus a DNS change — and without the join a spent approval would be re-enqueued once a minute for 24 hours and refused by the kernel every time, burying every real failure in the queue's log. It gives up after 24 hours for the same reason. Delivery is therefore at-least-once and execution is once-only; the worst case is a resume that starts a minute late.
...
`resumeAgent` flips the run to `running` before the approved tool executes, so from that point every failure has to land somewhere terminal. Everything from loading the parked run onwards sits in one try/catch — which logs the failure against the run and **rethrows**. The run is left exactly as it was and pg-boss retries. That is the whole of the in-request behaviour: a spent approval, an approval bound to a different `tool_use`, another organisation's run, a run some other approval is driving, a delivery that lost the claim — all the same answer. A stale approval must never be able to kill a run that is legitimately waiting or working.

The worker's `agent.resume` handler treats a run that is `completed`, `failed` **or `running`** as an idempotent no-op with a log line: only a parked run can be resumed, and a retry against one that is not buries the real failures. `running` is included deliberately. The kernel used to carry an in-band `failStrandedRun` that guessed, from the age of `metadata.resume.claimedAt`, whether a `running` run had been abandoned; nothing refreshes that timestamp while a resume works, so a redelivery landing more than five minutes into a legitimately long resume would fail a run that was still going — and with `finish`'s `status = 'running'` predicate the live run's real outcome would then be the one discarded. It is gone.

**One mechanism closes a stranded run, and it decides on evidence.** `agent-runs.stuck-sweep` (cron, every ten minutes) fails any run still `running` that has recorded no step for thirty minutes and whose `metadata.resume.claimedAt`, if it has one, is at least that old — writing `agent.run_stranded` to `audit_log` and notifying the owner. A run that is visibly working is never eligible however long it takes, which is what stops it killing a legitimately long Opus run. The status flip and its audit row are one transaction, so a `failed` run is never durable without its trail; the notification is best effort after the commit, because a retry would find the run already `failed` and write neither.

`finish` merges `metadata` rather than replacing it — it sets `pending` on a re-park and removes that one key on a finish — so `metadata.resume` survives, and a finished run still says which approval claimed it and when. That is the whole diagnostic the run trace has for a resume that went wrong.
...
| `cms_update_content` | **requires_approval** | One page on a client's CMS; the site ref likewise comes from our rows. Real WordPress provider once `SECRETS_ENCRYPTION_KEY` is set and the site has a stored application password; built per run for the run's own organisation (`scopedCmsProvider`) — see the note under this table. |

- **The two fix tools are mocks until their keys are set.** `dns_update_record` goes through a per-domain registry — real Hostinger or Cloudflare when `HOSTINGER_API_TOKEN` / `CLOUDFLARE_API_TOKEN` is set for the provider the domain row names, a mock otherwise; `cms_update_content` gets the real WordPress provider when `SECRETS_ENCRYPTION_KEY` is set and the site has a credential row. On a mock, approving writes the `dns_records` row, the `audit_log` entry and the run trace exactly as it does live — but no zone and no page is touched. Both `describeApproval` implementations read the adapter that will really run at describe time (for DNS, the registry's answer for that domain; for CMS, the provider built for the run's organisation) and say so on the approval card, so the correction is on the screen where the decision is made rather than only here.
